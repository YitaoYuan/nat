...
try:
    ...
    for i in range(worker_num):
        if worker_type[i] == 1: # to WAN, transition_type == 0/2
            bfrt.nat.pipe.Ingress.send_out.forward_table.add_with_set_egress_port(0, 0xD, 1, 0x1, 0, 0x0, worker_ip[i], 0xffffffff, 0, 0x0, 1, worker_port[i])
        if worker_type[i] == 0: # to LAN, transition_type == 1/3
            bfrt.nat.pipe.Ingress.send_out.forward_table.add_with_set_egress_port(1, 0xD, 1, 0x1, 0, 0x0, 0, 0x0, worker_ip[i], 0xffffffff, 1, worker_port[i])
