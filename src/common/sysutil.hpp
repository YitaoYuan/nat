...
nic_getbusinfo_by_name(const char* nic_name, char** businfo) {
    ...
    ifr.ifr_data = (__caddr_t) &drvinfo;
...
get_numanode_num() {
    ...
    int numanode_num = -1;
    while (fscanf(fp, "%[^:]%*c %[^\n]%*c", key, value) != EOF) {
        if(strcmp(key, "NUMA node(s)") == 0) {
            int ret = sscanf(value, "%d", &numanode_num);
            break;
        }
    }
    fclose(fp);
...
get_ip_nexthop (uint32_t dst_ip) {
    ...
    char buff [256] = {};
    bool chosen = 0;
    uint32_t chosen_ip_net = 0, chosen_ip_mask = 0, chosen_gateway = 0;
    if (fp != NULL) {
        int ignored = 0;
        ignored = fscanf(fp, "%*[^\n]%*c");
        while (fscanf(fp, "%[^\n]%*c", buff) != EOF) {
...
bool 
has_suffix(const char* str, const char* suf) {
