...
/*
// Consistent Hashing: Configuration (Host byte order)
typedef unsigned short lb_hash_t;

struct lb_set_t {
    ip_addr_t vip;
    vector<ip_addr_t> server_ip;
    u32 table_size; // power of 2
};

vector<lb_set_t>lb_sets;

// Consistent Hashing: Hash table (Net byte order) 
typedef vector<ip_addr_t> maglev_hash_table_t;

// dst vip -> server table (and then 4-tuple ---hash--> server ip)
unordered_map<ip_addr_t, maglev_hash_table_t> maglev_hash_table_map;
...
// 因为交换机使用4元组哈希，所以NF也得使用同样的方式
typedef my_hash<lb_4_tuple, SHARED_SWITCH_CRC_POLY> lb_hash_alg_t;


// 5-tuple -> entry. The IP of server must be VIP.
unordered_map<flow_id_t, flow_entry_t*> lb_map;
*/
